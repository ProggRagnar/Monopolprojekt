      ...
      (cond 
        ((eq? ägare false)
         ...
         (if (eq? (read) 'ja) ; Ändra till knapptryckning med 2 alternativ.
             (begin
               ...
               (- (send ägare get-pengar) pris) ;Hur kan jag set! current-player till differansen av detta?
               (display "Gatan köpt!")
        ...
        (else 
         (begin
           ...
           (- (send (send world1 current-player) get-pengar) hyra)
           (+ (send ägare get-pengar) hyra)))))))
